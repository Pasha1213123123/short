...
// import 'package:calkulator/utils/video_player_utils.dart';
...
// // Провайдер для PageController, чтобы AdSessionNotifier мог к нему обращаться
// final shortsViewerPageControllerProvider =
//     StateProvider<PageController?>((ref) => null);
...
//   // ИСПРАВЛЕНИЕ ЛОГИКИ СЧЕТЧИКА: Инкремент и проверка в одном месте
//   void checkAndShowAd() {
//     state = state.copyWith(videosWatched: state.videosWatched + 1); // Инкремент

//     final videosWatched = state.videosWatched;
//     if (videosWatched > 0 && videosWatched % AppConfig.videosBetweenAds == 0) {
//       if (kDebugMode) {
//         debugPrint("Ad Checkpoint REACHED. Videos Watched: $videosWatched");
//       }
...
//               // Получаем PageController и текущую страницу
//               final currentPage = _ref.read(shortsViewerCurrentPageProvider);
//               final pageController =
//                   _ref.read(shortsViewerPageControllerProvider);
...
//                 // ИСПРАВЛЕНИЕ "ЗАЛИПАНИЯ": Явный переход на НОВУЮ страницу
...
//                   videosWatched:
//                       0); // КЛЮЧЕВОЕ: СБРОС СЧЕТЧИКА ПОСЛЕ УСПЕШНОГО ПОКАЗА
//               if (kDebugMode) {
...
//               // Добавляем логику перехода после неудачи
//               final currentPage = _ref.read(shortsViewerCurrentPageProvider);
//               final pageController =
//                   _ref.read(shortsViewerPageControllerProvider);
...
//               state = state.copyWith(
//                   videosWatched: 0); // СБРОС СЧЕТЧИКА после неудачи показа
...
//     } catch (e, st) {
//       // ИСПРАВЛЕНО: Передаем строку "disposeAndRemoveKey"
//       AnalyticsService.logError(e, st, context: "disposeAndRemoveKey");
//     }
//     _cache.remove(key);
//     _disposeCounter++;
...
//     // ИСПРАВЛЕНИЕ КРИТИЧЕСКОЙ ОШИБКИ RIVERPOD:
//     // Весь код, изменяющий провайдеры, выносится в post-frame callback
//     WidgetsBinding.instance.addPostFrameCallback((_) {
//       // Устанавливаем PageController в провайдер
//       ref.read(shortsViewerPageControllerProvider.notifier).state =
//           _pageController;

//       // Инициализация состояний, которая должна была быть в initState
...
//     // Сброс провайдера PageController при удалении виджета
//     ref.read(shortsViewerPageControllerProvider.notifier).state = null;
...
//     // Получаем контроллер предыдущей страницы *ДО* обновления _currentPage
//     final previousController = ref.read(shortsViewerCurrentControllerProvider);
...
//       // Остановка предыдущего видео (для экономии трафика/ресурсов)
//       previousController?.safePause();
//     }

//     WidgetsBinding.instance.addPostFrameCallback((_) async {
//       // 1. Устанавливаем НОВЫЙ контроллер и запускаем его
...
//       // 2. Логика рекламы (checkAndShowAd) вызывается только в _playerStateListener
//     });
//   }
...
//                 // ИСПРАВЛЕНИЕ: Засчитываем первый просмотр сразу (так как он уже начался)
//                 adNotifier.checkAndShowAd();
//                 if (kDebugMode) {
//                   debugPrint(
//                       "Video Watched Counter initialized/checked (First video).");
//                 }
...
// // ----------------------------------------------------
// class ShortsPlayerPage extends ConsumerStatefulWidget {
//   final YoutubePlayerController controller;
//   final int pageIndex;
...
//   @override
//   ConsumerState<ShortsPlayerPage> createState() => _ShortsPlayerPageState();
// }

// class _ShortsPlayerPageState extends ConsumerState<ShortsPlayerPage>
//     with AutomaticKeepAliveClientMixin {
//   bool _isPlayerVisible = false;
//   bool _showControls = false;
//   Timer? _controlsTimer;

//   // Флаг для предотвращения повторного инкремента при зацикливании
//   bool _hasVideoBeenCounted = false;

//   // ИСПРАВЛЕНИЕ: Ключевое для плавной прокрутки
//   @override
...
//       _hasVideoBeenCounted =
//           false; // Сбрасываем флаг, когда получаем новый контроллер для новой страницы
//     }
...
//   void _attemptAggressivePlay() async {
...
//     // Используем metadata для доступа к videoId и duration.
//     final videoMetadata = widget.controller.metadata;
//     final videoDuration = videoMetadata.duration;
//     final videoId = videoMetadata.videoId;
...
//       const watchThreshold = 0.9; // 90% просмотра

//       if (currentPositionMs > (videoDurationMs * watchThreshold)) {
//         _hasVideoBeenCounted = true; // Считаем просмотр
//         adNotifier.checkAndShowAd(); // Инкремент счетчика и проверка рекламы
//         if (kDebugMode) {
//           final safeVideoId = videoId.isNotEmpty ? videoId : 'UNKNOWN';
//           debugPrint("Video Watched (Logic): $safeVideoId counted.");
//         }
//       }
//     }
...
//     // Используем initialVideoId для заглушки (thumbnail)
//     final videoId = widget.controller.initialVideoId;
